<script setup lang="ts">
	...
	**********************************************************/
	...
	const bNuPhoto = ref(false)
	const imgSvr = ref('')

	const loadData = async () => {
		let url= window.sessionStorage.getItem('liwaAPIsvr') + "/001_haveDetail.php?siteID="+siteID.value+"&mainID=" + mainID.value
	...
	const loadDetail1 = async () => {
		let url= window.sessionStorage.getItem('liwaAPIsvr') + "/001_haveOwner.php?siteID="+siteID.value+"&mainID=" + mainID.value
		const data = await useFetch(url, {method: 'GET'}, {refetch: true}).get().json()
		liwaDetail1.value = data.data.value.arrSQL
	}

	const loadDetail2 = async () => {
		let url= window.sessionStorage.getItem('liwaAPIsvr') + "/001_haveExportpath.php?siteID="+siteID.value+"&mainID=" + mainID.value
		const data = await useFetch(url, {method: 'GET'}, {refetch: true}).get().json()
	...
	onMounted(() => {
    	...
    	const title = useTitle(compName+`- 系統程式明細`)
		// 取得 mainID, 由mainID取得
