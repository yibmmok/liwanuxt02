<script setup lang="ts">
	/*********************************************************
	prog name: 貼文明細CRUD, author: James Lin, date: 2022/08/02
	To Do: 發佈日期與下架日期改格式
	**********************************************************/
	...
	//import Config008 from "../../components/008/Config008.vue"
	import TinyMceEditor from "@tinymce/tinymce-vue"

	const mainID = ref('')
	const progName = ref('貼文列表')
	...
	const loadData = async () => {
		let url= window.sessionStorage.getItem('liwaAPIsvr') + "/008_haveDetail.php?siteID=" + window.sessionStorage.getItem('liwaSiteID') + "&mainID=" + mainID.value
	...
	const loadDetail1 = async () => {
		// 取得討論區類別列表
		let url1 = window.sessionStorage.getItem('liwaAPIsvr') + "/008_haveItemType.php?siteID="+window.sessionStorage.getItem('liwaSiteID')
	...
	const saveData = async () => {
	    ...
	    if (!msg) {
		    if (liwaData.value.action == 'add') {
		    	...
		    	window.location.href = '/008/' + mainID.value 
		    }	    	
	    } else {
	    	showMsg('存檔錯誤', msg, 1)
	    }
	...
	const clearData = () => {
		...
		detailName.value = '新增貼文'
		stitle.value = '新增貼文'
		liwaData.value = tmpData		
	}

	onMounted(() => {
    	const title = useTitle(`貼文明細`)
		// 取得 mainID, 由mainID取得
		...
		<div v-if="detailName" class="bg-white m-2 rounded-xl py-1">
			...
			>
				...
				<div class="w-full h-[450px]">
					<div class="w-full h-8 text-sm font-bold text-slate-800">貼文內容</div>
